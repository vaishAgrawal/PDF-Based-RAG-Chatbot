...
class VectorStore:
    ...
    def add(self, documents: list[str], document_id: str) -> None:
        if not documents:
            return
        ...
        self.documents.extend(
            {"document_id": document_id, "text": document} for document in documents
        )
        self._save()

    def search(self, query: str, document_id: str, limit: int = 4) -> list[str]:
        if self.index is None or not self.documents:
            return []
        vector = np.asarray(self.embedder.encode([query]), dtype="float32")
        _, indices = self.index.search(vector, len(self.documents))
        return [
            self.documents[index]["text"]
            for index in indices[0]
            if index >= 0 and self.documents[index]["document_id"] == document_id
        ][:limit]
    ...
    def _load(self) -> None:
        ...
        if index_path.exists() and documents_path.exists():
            ...
            stored_documents = json.loads(documents_path.read_text(encoding="utf-8"))
            self.documents = [
                document
                if isinstance(document, dict)
                else {"document_id": "legacy", "text": document}
